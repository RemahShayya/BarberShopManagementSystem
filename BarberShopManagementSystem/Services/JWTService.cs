...
namespace BarberShopManagementSystem.API.Services
{
    public class JWTService
    {
        ...
        {
            this.config = config;
            this.jwtKey = new SymmetricSecurityKey(Encoding.UTF8.GetBytes(config["JwtToken:Token"]));
        ...
        public async Task<string> CreateJWT(User user)
        {
            var userClaims = new List<Claim>
            {
                ...
                new Claim(ClaimTypes.Email, user.Email?? "admins__account@@"),
            ...
            var tokenDescriptor = new SecurityTokenDescriptor
            {
                ...
                Expires = DateTime.UtcNow.AddHours(int.Parse(config["JwtToken:ExpiresInHours"])),
